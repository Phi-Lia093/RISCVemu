...
uint32_t mem_read8_unsigned(uint32_t addr) {
    if (addr >= MEM_SIZE) {
        // E
        return 0;
    }
...
uint32_t mem_read16_unsigned(uint32_t addr) {
    if (addr >= MEM_SIZE - 1) {
        // E
        return 0;
    }
    ...
    return (uint32_t)main_mem[addr] | 
           ((uint32_t)main_mem[addr+1] << 8);
}

uint32_t mem_read32_unsigned(uint32_t addr) {
    if (addr >= MEM_SIZE - 3) {
        // E
        return 0;
    }
...
int32_t mem_read8_signed(uint32_t addr) {
    if (addr >= MEM_SIZE) {
        // E
        return 0;
    }
...
int32_t mem_read16_signed(uint32_t addr) {
    if (addr >= MEM_SIZE - 1) {
        // E
        return 0;
    }
...
uint32_t mem_read32_signed(uint32_t addr) { // the same as mem_read_32_unsinged as 32bit has no extend problem
    if (addr >= MEM_SIZE - 3) {
        // E
        return 0;
    }
...
bool is_aligned(uint32_t addr, uint32_t size) {

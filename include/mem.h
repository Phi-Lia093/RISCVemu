...
static inline uint32_t mem_read8_unsigned(uint32_t addr)
{
    if (unlikely(addr >= MEM_SIZE)) return 0;
    return g_main_mem[addr];
}

static inline uint32_t mem_read16_unsigned(uint32_t addr)
{
...
static inline uint32_t mem_read32_unsigned(uint32_t addr)
{
...
static inline int32_t mem_read8_signed(uint32_t addr)
{
    if (unlikely(addr >= MEM_SIZE)) return 0;
    return (int32_t)(int8_t)g_main_mem[addr];
}

static inline int32_t mem_read16_signed(uint32_t addr)
{
    if (unlikely(addr >= MEM_SIZE - 1)) return 0;
    if (unlikely(addr & 0x1)) return 0;
...
static inline int32_t mem_read32_signed(uint32_t addr)
{
    if (unlikely(addr >= MEM_SIZE - 3)) return 0;
    if (unlikely(addr & 0x3)) return 0;
